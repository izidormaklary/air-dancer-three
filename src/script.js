...
const hairGeometry = new THREE.PlaneBufferGeometry(
  hairSizes.width,
  hairSizes.height,
  1,
  9
...
const skinGenerator = (sizingObject, geometryObject) => {
  ...
  geometryObject.setAttribute(
    "skinIndex",
    new THREE.Uint16BufferAttribute(meshSkinIndices, 4)
  );
  geometryObject.setAttribute(
    "skinWeight",
    new THREE.Float32BufferAttribute(meshSkinWeights, 4)
  );
};
...
const finalRotationLeft = new THREE.Quaternion().setFromAxisAngle(
  zAxis,
  (Math.PI / 180) * (bodySizes.boneCount / 5)
);
const finalRotationRight = new THREE.Quaternion().setFromAxisAngle(
  zAxis,
  (Math.PI / 180) * -(bodySizes.boneCount / 5)
);
const dampingRight = new THREE.Quaternion().setFromAxisAngle(
  zAxis,
  (Math.PI / 180) * -(bodySizes.boneCount / 15)
);
const dampingLeft = new THREE.Quaternion().setFromAxisAngle(
  zAxis,
  (Math.PI / 180) * (bodySizes.boneCount / 15)
);
// Generate Keyframes
...
for (let i = 0; i < KFCount; i++) {
  ...
  if (i === 0 || i === 3) {
    quaternionValueArrLeft.push(
      initialRotation.x,
      initialRotation.y,
      initialRotation.z,
      initialRotation.w
    );
    quaternionValueArrRight.push(
      initialRotation.x,
      initialRotation.y,
      initialRotation.z,
      initialRotation.w
    ...
    quaternionValueArrRight.push(
      finalRotationRight.x,
      finalRotationRight.y,
      finalRotationRight.z,
      finalRotationRight.w
    );
  }
}

// creating keyframe tracks then clips
const rotationKFLeft = new THREE.QuaternionKeyframeTrack(
  ".quaternion",
  KFIndexArr,
  quaternionValueArrLeft
);
const rotationKFRight = new THREE.QuaternionKeyframeTrack(
  ".quaternion",
  KFIndexArr,
  quaternionValueArrRight
);
...
for (let i = 1; i < bodyMesh.skeleton.bones.length - 1; i += 1) {
  const lerp =
    1.2 -
    Math.sqrt(bodyMesh.skeleton.bones.length - i) /
      bodyMesh.skeleton.bones.length;
  // console.log(Math.sqrt(i)/bodyMesh.skeleton.bones.length);
  ...
  clipActionLeft.setDuration(1);
  clipActionRight.setDuration(1);
  clipActionLeft.setLoop(THREE.LoopOnce);
  clipActionRight.setLoop(THREE.LoopOnce);
  clipActionLeft.setEffectiveWeight(
    i / bodyMesh.skeleton.bones.length / 2 + 0.5
  );
  clipActionRight.setEffectiveWeight(
    i / bodyMesh.skeleton.bones.length / 2 + 0.5
  );

  bodyBonesMixerArr.push({
    mixer: mixer,
    clipActionLeft: clipActionLeft,
    clipActionRight: clipActionRight,
  });
}
...
bodyBonesMixerArr[bodyBonesMixerArr.length - 1].mixer.addEventListener(
  "finished",
  (e) => {
...
const render = () => {
  ...
  arm1Mesh.skeleton.bones.slice(3).forEach((bone, index) => {
    bone.rotation.z =
      (Math.PI / 15) * Math.sin(clock.getElapsedTime() * 10 + index);
  });
  ...
  hairGroup.children.forEach((hair, index) => {
    hair.skeleton.bones.slice(0).forEach((bone) => {
      bone.rotation.x =
        (Math.PI / hairSizes.boneCount) *
        (Math.sin(clock.getElapsedTime() * 20 + index) / 5);
    });
  });
